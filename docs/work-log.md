...
## [2026-06-19] 법규 정확도 전면 업그레이드 (법령 오류 수정 + 누락 항목 추가)
- 변경파일: `lib/judge.ts`
- 근거 명세: `docs/law-review-spec.md` (Codex 작성, 2026-06-18)
- 핵심변경:
  - 🔴 `대지안의 피난통로` — 법령 제90조의2(오기) → **제41조**로 교체. 내용도 "막힌 복도" → "연면적 2,000㎡↑ 시 대지 내 3m 소화·피난 통로 확보" 기준으로 전면 재작성
  - 🔴 `막힌 복도 길이` — **별도 항목 신설** (공동주택 한정): 주택건설기준 제16조, 직통계단에서 20m 이하
  - 🟡 `가각지 건축제한` — 법령 건축법 제46조 → **시행령 제31조**, 각도별 기준(90°미만 4×4m, 90°~120°미만 3×3m) 상세화
  - 🟡 `장애인 편의시설` — 별표1 → **별표2**, 근린생활 300㎡ → **500㎡** 수정, 교육연구시설 추가, 공동주택 세대수 무관 전면 적용
  - 🟡 `지하층의 구조` — **비상탈출구 기준 추가** (거실 50㎡↑ 시 비상탈출구 또는 출구 2개소 이상)
  - 🟢 `복도너비` — **학교·교육연구 기준 추가** (양측 2.4m↑, 한쪽 1.8m↑), 법령에 피난방화규칙 제15조 추가
  - 🟠 `자동화재탐지설비` — 임계면적 용도별 정밀화 (강화용도 300㎡, 공동주택 600㎡·16세대, 일반 400㎡)
- 주의사항: `대지안의 피난통로` 항목명이 `대지안의 피난·소화 통로`로 변경됨. UI에서 해당 항목명 검색 시 주의.
